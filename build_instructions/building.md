...
- 2x M8 threaded rods of length 60cm.  
Take the main pump and mount it on its bracket. Push the threaded rods through the mount and tighten the screws fixing the position of the pump about 2cm away from the leftmost end of the rod. Note that connecting pipes should point upwards from the pump. It starts better when the gears inside of it are immersed into a small amount of remaining liquid, rather then when the gravity completely pulls the liquid out.  
...
Now that the assembly is somewhat rigid, mount the rest of the valves onto the rail about 5mm apart from one another.  
Take the fresh water pump and mount it with its bracket onto the rail, about 2cm away from the rightmost valve.  
Now mount the brackets for electronic box. You may have to push around the valves to make space for correct placement of the brackets. The distance between the brackets should be exactly the same as the distance between the mounting points of the box. About 1cm to the left of the first and the third valves is a good place for the box, this way the wires from each of the valves should reach their respective connectors without problems.  
...
- 2x threaded rods M8 of length 63cm then place parts in following order: nuts, support, nuts, holder, nuts, support, nuts
- 24x M8 nuts
- 2x M6 screws for mounting the force gauge
- 2x M5 screws for mounting magnetic holder to the force gauge
Take two threaded rods and place one nut on each of them about 8cm from the end. Push them through one magnetic holder and fix with two nuts on the other side. Place two more nuts about 8cm away from the previous two. Repeat nuts-holder-nuts-space pattern for a total of 4 holders. The fifth holder is for the filter and should be further away due to filter size and requires two additional supports due to its weight and instability. So, after the fourth holder keep about 15cm of space, then place parts in following order: nuts, support, nuts, holder, nuts, support, nuts. The idea is that each part is fixed on both sides and is in the contact with 4 nuts in total.  
Take the force gauge, and the pair of mounting brackets and fix the gauge to the very ens of the rods by threading each screw through two brackets into the gauge. Mind the orientation of the gauge, the arrow must be on the free side and should point downwards, and the angle must be fine tuned later so that the tank is as far from touching any of the walls as possible.  
...
If you have some special developing tank and need to make parts that fit it, refer to modifying_parts.md, it has description of how to make the right rod and cap. In this document it is supposed that you already have the parts that fit your tank perfectly.  
...
Cut 6 hoses for vessels, filter and tank. Their length needs to be enough to connect the upper rail and the lower rail and to allow comfortable handling of vessels, mounted to them. Recommended length is about 30cm. Now connect each hose to adapter as follows.  
...
Cut the hose for clean water input about the same size as the waste water hose, it shoud reach the source of clean water outside the machine. Connect it to the input of the filter pump. Cut about 30cm long hose for the filter input. Connect it to the output of the filter pump and fix its other side to the input of the filter. Put the level sensor on the filter input so the filter never overflows.
## Mounting main board
...
Put each pair of wires of each valve into holes in screw terminals and tighten them with screws. Polarity does not matter in case of valves. Connect the rightmost terminal to the power supply, here the polarity is important, make sure that the negative contact is on the left, and the positive is on the right.  
